defmodule Scandoc.Accounts.User do
  use Ecto.Schema
  ...
  """
  def zehut_changeset(user, attrs) do
    user
    |> cast(attrs, [:zehut])
    |> validate_zehut()
    |> case do
      %{changes: %{zehut: _}} = changeset -> changeset
      %{} = changeset -> add_error(changeset, :zehut, "did not change")
    end
  end
  ...
  """
  def valid_password?(%Scandoc.Accounts.User{hashed_password: hashed_password}, password)
