defmodule Mix.Tasks.Bootstrap.Gen.Html do
  @shortdoc "Generates controller, views, and context for an HTML resource"

  @moduledoc """
  Generates controller, views, and context for an HTML resource.

      mix bootstrap.gen.html Accounts User users name:string age:integer
  ...
  with their respective names and types. See `mix bootstrap.gen.schema`
  ...
  Alternatively, the `--context-app` option may be supplied to the generator:
      ...
      mix bootstrap.gen.html Sales User users --context-app warehouse
  ...
  module name, for example:
      ...
      mix bootstrap.gen.html Sales User users --web Sales
  ...
  use binary ids for primary keys, see `mix bootstrap.gen.schema` for more
  information.
  """
  use Mix.Task
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise("mix bootstrap.gen.html can only be run inside an application directory")
    end
  ...
  def copy_new_files(%Context{} = context, paths, binding) do
    ...
    Mix.Phoenix.copy_from(paths, "priv/templates/bootstrap.gen.html", binding, files)
  ...
  def inputs(%Schema{} = schema) do
    Enum.map(schema.attrs, fn
      ...
      {key, :integer} ->
        {label(key), ~s(<%= number_input f, #{inspect(key)}, class: "form-input" %>), error(key)}
  ...
  defp label(key) do
    ~s(<%= label f, #{inspect(key)}, class: \"col-form-label text-sm-right col-sm-2\" %>)
  end
